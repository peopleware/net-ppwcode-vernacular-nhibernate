...
namespace PPWCode.Vernacular.NHibernate.IV.DI
{
    ...
    {
        /// <summary>
        ///     Registers all NHibernate infrastructure services required to work with the PPWCode
        ///     NHibernate vernacular layer using a specified timestamp type.
        /// </summary>
        /// <typeparam name="TTimestamp">
        ///     The type of the timestamp, which must be a value type that implements
        ///     <see cref="IComparable{T}" /> and <see cref="IEquatable{T}" />.
        /// </typeparam>
        /// <param name="services">The <see cref="IServiceCollection" /> to add services to.</param>
        /// <param name="configure">
        ///     A delegate that receives an <see cref="NHibernateOptions" /> instance and is responsible for
        ///     configuring the NHibernate options, including at minimum
        ///     <see cref="NHibernateOptions.UseMappingAssemblies{T}" />,
        ///     <see cref="NHibernateOptions.UseHbmMapping{T}" />,
        ///     <see cref="NHibernateOptions.UseTimeProvider{T,TTimestamp}" />, and
        ///     <see cref="NHibernateOptions.UseIdentityProvider{T}" />.
        /// </param>
        ...
        /// </exception>
        /// <exception cref="PPWCode.Vernacular.Exceptions.V.Error">
        ///     Thrown when the <paramref name="configure" /> delegate does not call
        ///     <see cref="NHibernateOptions.UseMappingAssemblies{T}" />,
        ///     <see cref="NHibernateOptions.UseHbmMapping{T}" />,
        ///     <see cref="NHibernateOptions.UseTimeProvider{T,TTimestamp}" />, or
        ///     <see cref="NHibernateOptions.UseIdentityProvider{T}" />, as all four are mandatory.
        /// </exception>
        /// <remarks>
        ///     This method registers essential singleton services required for using NHibernate within
        ///     the application, including
        ///     - <see cref="IMappingAssemblies" />
        ///     - <see cref="IPpwHbmMapping" />
        ///     - <see cref="ITimeProvider{TTimestamp}" />
        ///     - <see cref="IIdentityProvider" />
        ///     - <see cref="IExceptionTranslator" />
        ///     - <see cref="INhProperties" />
        ///     - <see cref="INhConfiguration" />
        ///     - <see cref="INHibernateSessionFactory" />
        ///     All these registrations use <c>TryAdd</c> semantics, meaning that pre-existing application-specific
        ///     registrations take precedence. The method also allows for customization of NHibernate
        ///     behavior through the provided <paramref name="configure" /> option.
        /// </remarks>
        public static IServiceCollection AddNHibernate<TTimestamp>(this IServiceCollection services, Action<NHibernateOptions> configure)
            where TTimestamp : struct, IComparable<TTimestamp>, IEquatable<TTimestamp>
        {
            ...
            {
                throw new Error($"You must supply a dependency of type {nameof(ITimeProvider<TTimestamp>)}, using the {nameof(NHibernateOptions.UseTimeProvider)} method.");
            }
            ...
            services.TryAddSingleton(sp => (ITimeProvider<TTimestamp>)sp.GetRequiredService(options.TimeProvider));
